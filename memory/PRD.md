...
- Email + in-app notifications: Resend integrated. RESEND API KEY set in backend/.env — ACCOUNT IN TESTING MODE: delivers only to account owner (contact@bioexamprep.com) until domain verified at resend.com/domains; failures caught + logged gracefully. In-app: db.notifications, GET /api/notifications + read-all, NotificationsBell (unread badge, panel, 30s poll) in portal. Triggers: enrollment (student email + admin BCC + teacher in-app), payment success (student + admin BCC), assignment graded (email), announcement posted, live class scheduled (scoped).
